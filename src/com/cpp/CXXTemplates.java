 ...
 * <p>
 * <p>
 * This class holds all the templates that will be used when generating the actual wrapper code
 * <p>
 * =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
 */
...
public class CXXTemplates {
    ...
    public static final String WRAPPED_POINTER = "%qualified_name* %member_name = nullptr;";
